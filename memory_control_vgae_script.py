...
"""
Creates a Variational Graph Autoencoder (VGAE) model to learn the latent representation of nodes in a graph.
"""
...
def build_sparse_adjacency_from_embeddings(embeddings, threshold=0.25):
    ...
    adjacency_matrix = coo_matrix(
        (data, (row, col)), shape=(embeddings.shape[0], embeddings.shape[0])
    )
...
def train_vgae(data, model, beta, epochs=100, lr=0.01):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            print(
                f"Epoch {epoch}/{epochs}, Recon Loss: {recon_loss.item():.4f}, KL Loss: {kl_loss.item():.4f}"
            )
    print(
        f"Epoch {epoch}/{epochs}, Recon Loss: {recon_loss.item():.4f}, KL Loss: {kl_loss.item():.4f}"
    )
...
if __name__ == "__main__":
    ...
    # Initialize sparse adjacency matrix, can be replaced with custom function
    ...
    in_channels = control_embeddings_list.size(
        1
    ...
    np.save(
        "control_reconstructed_adjacency_matrix.npy",
        reconstructed_adjacency.cpu().detach().numpy(),
    )
    print("Reconstructed adjacency matrix:")
    print(reconstructed_adjacency)
    visualize_reconstructed_adjacency_matrix(
        reconstructed_adjacency.detach().cpu().numpy(), "control"
    )
    ...
    print(reconstructed_adjacency)
    visualize_reconstructed_adjacency_matrix(
        reconstructed_adjacency.detach().cpu().numpy(), "group2"
    )
    np.save(
        "group2_reconstructed_adjacency_matrix.npy",
        reconstructed_adjacency.cpu().detach().numpy(),
    )
    ...
    print(reconstructed_adjacency)
    visualize_reconstructed_adjacency_matrix(
        reconstructed_adjacency.detach().cpu().numpy(), "group3"
    )
    np.save(
        "group3_reconstructed_adjacency_matrix.npy",
        reconstructed_adjacency.cpu().detach().numpy(),
    )
