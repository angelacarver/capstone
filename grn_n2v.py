...
def load_and_preprocess_data(filepaths: list, batch_labels: list):
    ...
    for filepath in filepaths:
        adata = sc.read_10x_mtx(
            filepath, var_names="gene_symbols", cache=True
        )  # ('C:\\Users\\a303\\Downloads\\alzheimers_data\\', var_names='gene_symbols', cache=True)
        adata_objects.append(adata)
    adata = ad.concat(
        adata_objects, join="outer", label="batch", keys=batch_labels, index_unique="-"
    )
    ...
    genes_to_keep = ~(adata.var["mt"] | adata.var["hb"] | adata.var["ribo"])
    ...
    sc.pl.pca(
        adata,  # color=["sample", "sample", "pct_counts_mt", "pct_counts_mt"], #look into color issues
...
def cell_type_annotation(adata, filepath):
    ...
    for i in range(n_plots):
        subset_markers = available_markers[
            i * markers_per_plot : (i + 1) * markers_per_plot
        ]
    ...
    sc.tl.rank_genes_groups(
        adata, groupby="leiden_res_0.50", method="t-test"
    )  # number of clusters in the ranking doesn't match number of clusters in leiden .5 graph
    ...
    control_data = adata[
        (adata.obs["batch"] == "control")
        & (adata.obs["leiden_res_0.50"].isin(clusters_of_interest))
    ]

    group2_data = adata[
        (adata.obs["batch"] == "treatment1")
        & (adata.obs["leiden_res_0.50"].isin(clusters_of_interest))
    ]

    group3_data = adata[
        (adata.obs["batch"] == "treatment2")
        & (adata.obs["leiden_res_0.50"].isin(clusters_of_interest))
    ]
...
def apply_node2vec(G: nx.Graph) -> dict:
    ...
    )  # apply Node@Vec on constructed graph
    model = node2vec.fit(
        window=10, min_count=1, batch_words=4
    )  # fit the model to get node embeddings
    embeddings = {
        node: model.wv[node] for node in G.nodes()
    }  # get embeddings as a dictionary {gene: embedding_vector}
    print(embeddings)
    return embeddings
...
# Visualization and Interpretation of tsne
def visualize_graph(node_embeddings):
    ...
    plt.scatter(
        node_embeddings_tsne[:, 0], node_embeddings_tsne[:, 1], cmap="jet", alpha=alpha
    )
...
def ANOVA(control_embeddings, treatment1_embeddings, treatment2_embeddings):
    ...
    adjusted_p_values = multipletests(p_values, method="fdr_bh")[
        1
...
# Extract top genes for each significant dimension
def extract_top_genes(significant_dims, embeddings):
    overall_top_genes = []
    for dim in significant_dims:
        top_genes = np.argsort(-np.abs(embeddings[:, dim]))[
            :10
        ]  # Top 10 genes by absolute value
        print(
            f"Significant dimension {dim}: Top contributing genes {adata.var_names[top_genes]}"
        )
...
def calculate_grn_metrics(grn_control, grn_treatment1):
    ...
    contingency_table = [
        ...
        [
            len(edges_treatment) - overlap,
            len(genes) ** 2 - len(edges_control) - len(edges_treatment) + overlap,
        ],
    ]
...
# Calculate node-level metrics for control and treatment networks
...
def louvain_subnetwork_analysis():
    ...
    community_degrees_control = [
        np.mean([grn_control.degree[n] for n in c]) for c in communities_control
    ]
    community_degrees_treatment = [
        np.mean([grn_treatment1.degree[n] for n in c]) for c in communities_treatment
    ]
...
# Visualize node degree differences
def visualize_node_degree_differences():
    # Degree differences
    degree_diff = {
        gene: metrics_treatment1["degree"].get(gene, 0)
        - metrics_control["degree"].get(gene, 0)
    ...
    nx.draw(
        grn_control,
        pos,
        node_color=list(degree_diff.values()),
...
# Using Jaccard similarity for edge overlap
...
def cluster_similarity(clusters1, clusters2):
    ...
    node_to_cluster1 = {
        node: i for i, cluster in enumerate(clusters1) for node in cluster
    }
    node_to_cluster2 = {
        node: i for i, cluster in enumerate(clusters2) for node in cluster
    }
...
if __name__ == "__main__":
    control_filepath = "C:\\Users\\a303\\Downloads\\alzheimers_data\\data\\control"  # change to your file path
    group2_filepath = "C:\\Users\\a303\\Downloads\\alzheimers_data\\data\\group2"  # change to your file path
    group3_filepath = "C:\\Users\\a303\\Downloads\\alzheimers_data\\data\\group3"  # change to your file path
    # combined group
    control_data = load_and_preprocess_data(
        [control_filepath], batch_labels=["control"]
    )
    group2_data = load_and_preprocess_data(
        [group2_filepath], batch_labels=["treatment1"]
    )
    group3_data = load_and_preprocess_data(
        [group3_filepath], batch_labels=["treatment2"]
    )
