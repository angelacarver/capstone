...
def degree_distribution(
    ...
    plt.hist(
        ...
        bins=20,
        alpha=0.7,
        label=f"{network1_name} Degree Distribution",
    )
    plt.xlabel("Degree")
    ...
    plt.hist(
        ...
        bins=20,
        alpha=0.7,
        label=f"{network2_name} Degree Distribution",
    )
    plt.xlabel("Degree")
    plt.ylabel("Frequency")
    plt.title("Degree Distribution")
    ...
    plt.hist(
        ...
        bins=20,
        alpha=0.7,
        label=f"{network3_name} Degree Distribution",
    )
    plt.xlabel("Degree")
    plt.ylabel("Frequency")
    plt.title("Degree Distribution")
...
def node_centrality(
    network1, network2, network3, network1_name, network2_name, network3_name
):
    """
    ...
    for network in [network1, network2, network3]:
        ...
        print(
            f"Top 5 Nodes by Degree Centrality for network {i}: {sorted(deg_centrality.items(), key=lambda x: x[1], reverse=True)[:5]}"
        )
        betweenness_centrality = nx.betweenness_centrality(network)
        print(
            f"Top 5 Nodes by Betweenness Centrality for network {i}: {sorted(betweenness_centrality.items(), key=lambda x: x[1], reverse=True)[:5]}"
        )
        i += 1
...
def plot_embeddings(embedding1, embedding2):
    ...
    # Plot
    plt.scatter(
        pca[: len(embedding1), 0],
        pca[: len(embedding1), 1],
        label="Treatment1",
        alpha=0.6,
    )
    plt.scatter(
...
def jaccard_similarity(embedding1, embedding2):
    ...
    jaccard = jaccard_score(
        neighbors1[node_index], neighbors2[node_index], average="micro"
    )
...
def embeddings_to_networkx(network_embeddings):
    reconstructed_adj = (
        network_embeddings @ network_embeddings.T
...
if __name__ == "__main__":
    ...
    # Make sure to load the pickle files in the same directory as this script and are generated from the previous scripts
    ...
    vgae_control_embeddings = np.load(
        "vgae_control_node_embeddings.npy", allow_pickle=True
    )
    vgae_treat1_embeddings = np.load(
        "vgae_group2_node_embeddings.npy", allow_pickle=True
    )
    vgae_treat2_embeddings = np.load(
        "vgae_group3_node_embeddings.npy", allow_pickle=True
    )

    # convert to networkx objects, uncomment if you want to save the networkx objects
    ...
    # open networkx objects, comment out if you want to save the networkx objects
    ...
    print("control")
    f_stat, p_value = degree_distribution(
        corr_control,
        node2vec_control_network,
        vgae_control_network,
    ...
    f_stat, p_value = degree_distribution(
        corr_treat1,
        node2vec_treat1_network,
        vgae_treat1_network,
        "Treatment 1 Pearson correlation",
        "Treatment 1 node2vec",
        "Treatment 1 VGAE",
    )
    print("treatment 2")
    f_stat, p_value = degree_distribution(
        corr_treat2,
        node2vec_treat2_network,
        vgae_treat2_network,
        "Treatment 2 Pearson correlation",
        "Treatment 2 node2vec",
        "Treatment 2 VGAE",
    )
    ...
    print("control")
    clustering_coefficients(
        corr_control, node2vec_control_network, vgae_control_network
    )
    print("treatment 1")
    ...
    print("control")
    modularity_scores = modularity_score(
        corr_control, node2vec_control_network, vgae_control_network
    )
    print("treatment 1")
    modularity_scores = modularity_score(
        corr_treat1, node2vec_treat1_network, vgae_treat1_network
    )
    print("treatment 2")
    modularity_scores = modularity_score(
        corr_treat2, node2vec_treat2_network, vgae_treat2_network
    )
